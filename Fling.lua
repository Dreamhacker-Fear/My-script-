...
--// Patched: avatar → REAPEROFCHRISTUS (8407673016), GUI centered
...
frame:GetPropertyChangedSignal("Position"):Connect(function()
    ...
    if pos.X.Offset < minX then
        frame.Position = UDim2.new(0, minX, pos.Y.Scale, pos.Y.Offset)
    elseif pos.X.Offset > maxX then
        frame.Position = UDim2.new(0, maxX, pos.Y.Scale, pos.Y.Offset)
    end

    local minY = 0
    local maxY = screenSize.Y - frameSize.Y
    if pos.Y.Offset < minY then
        frame.Position = UDim2.new(frame.Position.X.Scale, frame.Position.X.Offset, 0, minY)
    elseif pos.Y.Offset > maxY then
        frame.Position = UDim2.new(frame.Position.X.Scale, frame.Position.X.Offset, 0, maxY)
    end
end)
...
phase4Button1.Position = UDim2.new(0.05, 0, 0, 40)
...
phase4Button2.Position = UDim2.new(0.05, 0, 0, 80)
...
phase4Button3.Position = UDim2.new(0.05, 0, 0, 120)
...
phase4Button4.Position = UDim2.new(0.05, 0, 0, 160)
...
local function toggleMinimize()
    ...
    local elementsToHide = {
        ...
        phase4Button1, phase4Button2, phase4Button3, phase4Button4,
...
local function manageDensity(character, enable)
    ...
    for _, conn in pairs(densityConnections) do
        conn:Disconnect()
    end
    densityConnections = {}
...
phase4Button1.MouseButton1Click:Connect(function()
    ...
    if character then
        manageDensity(character, densityEnabled)
    end
...
phase4Button2.MouseButton1Click:Connect(function()
    ...
    if gravityEnabled then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Gravity",
            Text = "Enabled (Gravity: 1000000000000)",
            Duration = 2
        })
    else
        if gravityConnection then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Gravity",
            Text = "Disabled (Gravity restored to 196.2)",
            Duration = 2
        })
    end
end)
...
phase2Button2.MouseButton1Click:Connect(function()
    ...
    if spawnpointActive then
        ...
        if spawnCharacterConn then
            spawnCharacterConn:Disconnect()
            spawnCharacterConn = nil
        end
    end
end)
...
phase2Button5.MouseButton1Click:Connect(function()
    ...
    if infinitePositionEnabled then
        ...
        if respawnConnectionIP then
            respawnConnectionIP:Disconnect()
            respawnConnectionIP = nil
        end
...
phase3Button2.MouseButton1Click:Connect(function()
    ...
    if antiConveyorEnabled then
        ...
        if antiConveyorConnection then
            antiConveyorConnection:Disconnect()
            antiConveyorConnection = nil
        end
...
btnCorner:Clone().Parent = phase4Button1
btnCorner:Clone().Parent = phase4Button2
btnCorner:Clone().Parent = phase4Button3
btnCorner:Clone().Parent = phase4Button4
...
-- ✅ FIX: Use REAPEROFCHRISTUS UserId instead of the original author
...
phase4Button3.MouseButton1Click:Connect(function()
    ...
    if antiTornadoEnabled then
        ...
        if antiTornadoConnection then
            antiTornadoConnection:Disconnect()
            antiTornadoConnection = nil
        end
